...
`verify-input` is the guided four-axis live-stick check; it never creates a virtual controller and exits nonzero if any axis does not visibly change. The default mapping is stored in `%LOCALAPPDATA%\RCN-FPVSkyDive\state\mapping.conf`. Each Xbox axis supports `invert`, `deadzone`, `trim`, `saturation`, and `curve`; invalid values are clamped and buttons remain clear. `diagnose` also reports Windows, signed DJI driver records, live-frame probing, Steam detection, startup state, process identity, and the last 100 log lines.
