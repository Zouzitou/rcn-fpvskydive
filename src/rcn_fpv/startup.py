...
def choose_startup_method(scheduled_task_available: bool, startup_folder_available: bool):
    if scheduled_task_available:
        return StartupPlan("scheduled-task", "RCN-FPVSkyDive\Bridge", "verified per-user task available")
    if startup_folder_available:
        return StartupPlan("startup-folder", "RCN-FPVSkyDive\Bridge.lnk", "scheduled task unavailable; using current-user fallback")
