...
| Missing VCOM driver | Ports-class driver is absent | Put a verified official DJI driver package in `%LOCALAPPDATA%\RCN-FPVSkyDive\drivers`, then run `rcn-fpv driver-install --inf <path-to-inf>` and approve its UAC prompt. The command verifies the active driver afterwards. `repair` restores the app and startup registration; it never changes drivers. |
